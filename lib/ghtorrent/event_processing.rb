module GHTorrent
  module EventProcessing
    ...
    def PushEvent(data)
      ...
      return unless data['payload']['commits'].size >= 20

      info "PushEvent #{data['id']} has >= 20 commits."

      owner        = data['repo']['name'].split(%r{/})[0]
      repo         = data['repo']['name'].split(%r{/})[1]
      last_sha     = data['payload']['commits'].last['url'].split(%r{/})[7]
      ...
      while true
        ...
        commits.each do |c|
          ...
          sha_not_exist = ght.db.from(:commits, :project_commits, :projects, :users)\
                             .where(Sequel.qualify('projects',
                                                   'id') => Sequel.qualify('project_commits', 'project_id'))\
    ...
    def WatchEvent(e)
      owner      = e['repo']['name'].split(%r{/})[0]
      repo       = e['repo']['name'].split(%r{/})[1]
    ...
    def MemberEvent(data)
      ...
      ght.transaction do
        ...
        return if project.nil? or new_user.nil?
    ...
    def PullRequestReviewCommentEvent(data)
      ...
      pullreq_id = data['payload']['comment']['_links']['pull_request']['href'].split(%r{/})[-1]
